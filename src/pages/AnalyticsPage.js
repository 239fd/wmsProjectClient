...
const AnalyticsPage = () => {
    ...
    useEffect(() => {
        ...
        analyticsService.getWarehousesOrgSummary(orgId)
            .then((whs) => {
                if (!cancelled && whs) setWarehousesSummary(whs);
            })
...
const OverviewTab = ({
                         loading, warehousesSummary, opsDynamics, abcDistribution, periodLabel,
                                            ...
                                            <CardContent>
                                                <Stack direction="row" alignItems="center" spacing={1.5} mb={2}>
                                                    ...
                                                    <Typography variant="subtitle1" fontWeight={700}
                                                                noWrap>{w.name || '—'}</Typography>
                                                </Stack>
                                                <Stack spacing={1}>
                                                    <Stack direction="row" justifyContent="space-between">
                                                        <Typography variant="body2"
                                                                    color="text.secondary">Стеллажей</Typography>
                                                        <Typography variant="body2"
                                                                    fontWeight={600}>{racksCount ?? '—'}</Typography>
                                                    </Stack>
                                                    <Stack direction="row" justifyContent="space-between">
                                                        <Typography variant="body2" color="text.secondary">Ячеек
                                                            занято</Typography>
                                                        <Typography variant="body2" fontWeight={600}>
                                                    ...
                                                    <Box>
                                                        <Stack direction="row" justifyContent="space-between" mb={0.5}>
                                                            <Typography variant="body2"
                                                                        color="text.secondary">Заполненность</Typography>
                                                            <Typography variant="body2"
                                                                        fontWeight={700}>{fill}%</Typography>
                                                        </Stack>
                                                        <LinearProgress
                                                            variant="determinate"
...
const OperationsTab = ({dateRange}) => {
    ...
    useEffect(() => {
        setPage(0);
    }, [typeFilter, whFilter, dateRange?.startDate, dateRange?.endDate]);
                                        ...
                                        <TableCell>
                                            ...
                                            {emp?.email && (
                                                <Typography variant="caption"
                                                            color="text.secondary">{emp.email}</Typography>
                                            )}
                                        </TableCell>
                                        <TableCell>
                        ...
                        <Card variant="outlined" sx={{borderRadius: 3, mb: 3, bgcolor: '#f5f7fa'}}>
                            <CardContent>
                                ...
                                <Grid container spacing={2}>
                                    ...
                                    <Grid size={{xs: 6, md: 3}}>
                                        <Typography variant="caption" color="text.secondary">Заполненность</Typography>
                                        <Typography variant="h6"
                                                    fontWeight={700}>{aggregated.utilizationPercent}%</Typography>
                                    </Grid>
                                </Grid>
                            </CardContent>
                        </Card>
                                            ...
                                            <Grid container spacing={1.5}>
                                                <Grid size={{xs: 6}}>
                                                    <Typography variant="caption"
                                                                color="text.secondary">Стеллажей</Typography>
                                                    <Typography variant="body2"
                                                                fontWeight={700}>{racksCount ?? '—'}</Typography>
                                                </Grid>
                                                <Grid size={{xs: 6}}>
                                                    <Typography variant="caption" color="text.secondary">Ячеек
                                                        занято</Typography>
                                                    <Typography variant="body2" fontWeight={700}>
                                                        {occupied ?? '—'}{total != null && ` / ${total}`}
                                                    </Typography>
                                                ...
                                                <Grid size={{xs: 6}}>
                                                    <Typography variant="caption" color="text.secondary">Уникальных
                                                        товаров</Typography>
                                                    <Typography variant="body2"
                                                                fontWeight={700}>{w.uniqueProducts ?? '—'}</Typography>
                                                </Grid>
                                                <Grid size={{xs: 6}}>
                                                    <Typography variant="caption" color="text.secondary">Операций ·
                                                        месяц</Typography>
                                                    <Typography variant="body2"
                                                                fontWeight={700}>{w.monthlyOperations ?? w.operationsLast30Days ?? '—'}</Typography>
                                                </Grid>
                                            </Grid>
                    ...
                    return (
                        <Grid size={{xs: 6, md: 3}} key={key}>
                            <Card variant="outlined" sx={{borderRadius: 3, bgcolor: m.bg}}>
                                <CardContent>
                                    <Typography variant="caption"
                                                sx={{color: m.color, fontWeight: 700, textTransform: 'uppercase'}}>
                                        {m.label}
                                    </Typography>
                            ...
                            {data.map((r, i) => {
                                ...
                                return (
                                    <TableRow key={r.batchId || i} hover>
                                        ...
                                        <TableCell>
                                            <Typography variant="body2"
                                                        fontWeight={600}>{r.productName || '—'}</Typography>
                                        </TableCell>
                                        <TableCell>
                                            <Typography variant="caption" sx={{fontFamily: 'monospace'}}>
                                                {r.sku || '—'}
                                        ...
                                        <TableCell>
                                            {r.storageConditions ? (
                                                <Chip size="small"
                                                      variant="outlined" {...enumChipProps('StorageConditions', r.storageConditions)} />
                                            ) : '—'}
                                        </TableCell>
                                    </TableRow>
                                );
